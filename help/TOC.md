---
product: adobe campaign
sub-product: adobe campaign標準
audience: 使用者
user-guide-title: Adobe Campaign Standard說明
translation-type: tm+mt
source-git-commit: 3b7da7df5092476be4c6acc21b2ad2472a80da83

---


# Adobe Campaign Standard說明 {#using}

+ [產品文件](campaign-standard-home.md)
+ 發行說明 {#release-notes}
   + [發行說明](rn/using/release-notes.md)
   + [發行說明2018](rn/using/release-notes-2018.md)
   + [2017年發行說明](rn/using/release-notes-2017.md)
   + [發行說明2015-2016](rn/using/release-notes-2015-2016.md)
   + [文件更新](rn/using/documentation-updates.md)
+ 快速入門 {#getting-started}
   + 關於Adobe Campaign {#about-adobe-campaign}
      + [宣傳協調](start/using/campaign-orchestration.md)
      + [細分與鎖定](start/using/segmentation-and-targeting.md)
      + [動態與個人化內容](start/using/dynamic-and-personalized-content.md)
      + [追蹤、監控和報告](start/using/tracking--monitoring-and-reporting.md)
      + [與Adobe解決方案整合](start/using/integrating-with-adobe-solutions.md)
   + 探索介面 {#discovering-the-interface}
      + [關於介面](start/using/about-the-interface.md)
      + [介面說明](start/using/interface-description.md)
      + [相容瀏覽器](start/using/compatible-browsers.md)
      + [自訂清單](start/using/customizing-lists.md)
   + 行銷計劃 {#marketing-plans}
      + [計劃與宣傳活動](start/using/programs-and-campaigns.md)
      + [行銷活動](start/using/marketing-activities.md)
      + [時間軸](start/using/timeline.md)
   + 管理範本 {#managing-templates}
      + [關於範本](start/using/about-templates.md)
      + [建立新範本](start/using/creating-a-new-template.md)
      + [建立多語言範本](start/using/creating-a-multilingual-template.md)
      + [使用範本](start/using/using-a-template.md)
   + 教學課程 {#tutorials}
      + [逐步指南](start/using/step-by-step-guides.md)
      + [教學影片](start/using/how-to-videos.md)
+ 個人檔案與受眾 {#profiles-and-audiences}
   + [關於個人檔案和受眾](audiences/using/about-profiles-and-audiences.md)
   + 管理個人檔案 {#managing-profiles}
      + [關於描述檔](audiences/using/about-profiles.md)
      + [建立描述檔](audiences/using/creating-profiles.md)
      + [編輯描述檔](audiences/using/editing-profiles.md)
      + [整合式客戶個人檔案](audiences/using/integrated-customer-profile.md)
      + [活動中設定檔](audiences/using/active-profiles.md)
   + 管理受眾 {#managing-audiences}
      + [關於受眾](audiences/using/about-audiences.md)
      + [建立觀眾](audiences/using/creating-audiences.md)
      + [在訊息中選取對象](audiences/using/selecting-an-audience-in-a-message.md)
   + 管理訂閱 {#managing-subscriptions}
      + [關於訂閱](audiences/using/about-subscriptions.md)
      + [建立服務](audiences/using/creating-a-service.md)
      + [促銷服務](audiences/using/promoting-a-service.md)
      + [監控訂閱](audiences/using/monitoring-subscriptions.md)
   + 瞭解選擇加入和退出程序 {#understanding-opt-in-and-opt-out-processes}
      + [關於促銷活動中選擇加入和退出](audiences/using/about-opt-in-and-opt-out-in-campaign.md)
      + [管理促銷活動中的選擇加入和退出](audiences/using/managing-opt-in-and-opt-out-in-campaign.md)
      + [識別傳送的退出描述檔](audiences/using/identifying-opt-out-profiles-for-a-delivery.md)
+ 通訊管道 {#communication-channels}
   + 關於通訊管道 {#about-communication-channels}
      + [探索通訊通道](channels/using/discovering-communication-channels.md)
      + [傳送訊息的關鍵步驟](channels/using/key-steps-to-send-a-message.md)
      + [存取訊息](channels/using/accessing-messages.md)
      + [訊息控制面板](channels/using/message-dashboard.md)
   + 電子郵件訊息 {#email-messages}
      + [關於電子郵件](channels/using/about-emails.md)
      + [建立電子郵件](channels/using/creating-an-email.md)
      + [設計A/B測試電子郵件](channels/using/designing-an-a-b-test-email.md)
      + [建立多語言電子郵件](channels/using/creating-a-multilingual-email.md)
   + SMS訊息 {#sms-messages}
      + [關於SMS訊息](channels/using/about-sms-messages.md)
      + [建立SMS訊息](channels/using/creating-an-sms-message.md)
      + [個人化SMS訊息](channels/using/personalizing-sms-messages.md)
      + [管理傳入的SMS](channels/using/managing-incoming-sms.md)
   + 推播通知 {#push-notifications}
      + [關於推播通知](channels/using/about-push-notifications.md)
      + [準備和傳送推播通知](channels/using/preparing-and-sending-a-push-notification.md)
      + [自訂推播通知](channels/using/customizing-a-push-notification.md)
      + [建立多語言推播通知](channels/using/creating-a-multilingual-push-notification.md)
      + [根據行動應用程式資料建立和更新描述檔資訊](channels/using/updating-profile-with-mobile-app-data.md)
   + 應用程式內傳訊 {#in-app-messaging}
      + [關於應用程式內傳訊](channels/using/about-in-app-messaging.md)
      + [準備及傳送應用程式內訊息](channels/using/preparing-and-sending-an-in-app-message.md)
      + [自訂應用程式內訊息](channels/using/customizing-an-in-app-message.md)
   + 直接郵件 {#direct-mail}
      + [關於直接郵件](channels/using/about-direct-mail.md)
      + [返回傳送者](channels/using/return-to-sender.md)
      + [建立直接郵件](channels/using/creating-the-direct-mail.md)
      + [定義直接郵件對象](channels/using/defining-the-direct-mail-audience.md)
      + [定義直接郵件內容](channels/using/defining-the-direct-mail-content.md)
      + [工作流程中的直接郵件範例](channels/using/example-of-direct-mail-in-a-workflow.md)
   + 交易訊息 {#transactional-messaging}
      + [關於交易訊息](channels/using/about-transactional-messaging.md)
      + [事件交易訊息](channels/using/event-transactional-messages.md)
      + [個人檔案交易訊息](channels/using/profile-transactional-messages.md)
      + [交易推播通知](channels/using/transactional-push-notifications.md)
      + [後續訊息](channels/using/follow-up-messages.md)
   + 著陸頁面 {#landing-pages}
      + [關於著陸頁面](channels/using/about-landing-pages.md)
      + [著陸頁面限制](channels/using/landing-page-limitations.md)
      + [設計登陸頁面](channels/using/designing-a-landing-page.md)
      + [共用登陸頁面](channels/using/sharing-a-landing-page.md)
      + [資料儲存與協調](channels/using/data-storage-and-reconciliation.md)
      + [設定雙重加入程序](channels/using/setting-up-a-double-opt-in-process.md)
+ 設計內容 {#designing-content}
   + 關於內容設計 {#about-content-design}
      + [在Adobe Campaign中設計內容](designing/using/designing-content-in-adobe-campaign.md)
      + [內容設計最佳實務](designing/using/content-design-best-practices.md)
   + 編輯電子郵件內容 {#editing-email-content}
      + [關於電子郵件內容設計](designing/using/about-email-content-design.md)
      + [定義電子郵件結構](designing/using/defining-the-email-structure.md)
      + [編輯電子郵件樣式](designing/using/editing-email-styles.md)
   + 編輯著陸頁面內容 {#editing-landing-page-content}
      + [關於著陸頁面內容設計](designing/using/about-landing-page-content-design.md)
      + [著陸頁面內容編輯器介面](designing/using/landing-page-content-editor-interface.md)
      + [管理著陸頁面結構與樣式](designing/using/managing-landing-page-structure-and-style.md)
      + [變更著陸頁面表單資料屬性](designing/using/changing-a-landing-page-form-data-properties.md)
   + 編輯SMS和推播內容 {#editing-sms-and-push-content}
      + [關於SMS和推播內容設計](designing/using/about-sms-and-push-content-design.md)
      + [SMS和推播內容編輯器介面](designing/using/sms-and-push-content-editor-interface.md)
   + 載入內容 {#loading-content}
      + [選取現有內容](designing/using/selecting-an-existing-content.md)
      + [從檔案匯入內容](designing/using/importing-content-from-a-file.md)
      + [從URL匯入內容](designing/using/importing-content-from-a-url.md)
      + [使用電子郵件設計工具編輯現有內容](designing/using/editing-existing-contents-with-the-email-designer.md)
   + 使用影像 {#using-images}
      + [插入影像](designing/using/inserting-images.md)
      + [設定影像屬性](designing/using/setting-up-image-properties.md)
      + [使用Adobe Creative SDK修改影像](designing/using/modifying-images-with-the-adobe-creative-sdk.md)
      + [個人化影像來源](designing/using/personalizing-an-image-source.md)
   + 個人化內容 {#personalizing-content}
      + [關於個人化](designing/using/about-personalization.md)
      + [插入個人化欄位](designing/using/inserting-a-personalization-field.md)
      + [新增內容區塊](designing/using/adding-a-content-block.md)
      + [個人化傳送者](designing/using/personalizing-the-sender.md)
      + [個人化電子郵件的主旨行](designing/using/personalizing-the-subject-line-of-an-email.md)
      + [範例：電子郵件個人化](designing/using/example--email-personalization.md)
   + 定義條件式內容 {#defining-conditional-content}
      + [定義可見度條件](designing/using/defining-a-visibility-condition.md)
      + [在電子郵件中定義動態內容](designing/using/defining-dynamic-content-in-an-email.md)
      + [在登陸頁面中定義動態內容](designing/using/defining-dynamic-content-in-a-landing-page.md)
      + [定義動態文字](designing/using/defining-dynamic-text.md)
   + 管理連結 {#managing-links}
      + [插入連結](designing/using/inserting-a-link.md)
      + [個人化URL](designing/using/personalizing-urls.md)
      + [關於追蹤的URL](designing/using/about-tracked-urls.md)
+ 測試與傳送 {#testing-and-sending}
   + [關於使用Campaign傳送訊息](sending/using/about-sending-messages-with-campaign.md)
   + 準備和測試訊息 {#preparing-and-testing-messages}
      + [預覽訊息](sending/using/previewing-messages.md)
      + [準備傳送](sending/using/preparing-the-send.md)
      + [管理測試設定檔並傳送校樣](sending/using/managing-test-profiles-and-sending-proofs.md)
      + [電子郵件演算](sending/using/email-rendering.md)
   + 隱藏訊息 {#sheduling-messages}
      + [關於排程訊息](sending/using/about-scheduling-messages.md)
      + [最佳化傳送時間](sending/using/optimizing-the-sending-time.md)
      + [在收件者的時區傳送訊息](sending/using/sending-messages-at-the-recipient-s-time-zone.md)
      + [計算傳送日期](sending/using/computing-the-sending-date.md)
   + 傳送和追蹤訊息 {#sending-and-tracking-messages}
      + [確認傳送](sending/using/confirming-the-send.md)
      + [追蹤訊息](sending/using/tracking-messages.md)
   + 監控訊息 {#monitoring-messages}
      + [監控傳送](sending/using/monitoring-a-delivery.md)
      + [在發生失敗時接收警報](sending/using/receiving-alerts-when-failures-happen.md)
      + [瞭解傳遞失敗](sending/using/understanding-delivery-failures.md)
      + [瞭解隔離管理](sending/using/understanding-quarantine-management.md)
+ 報告功能 {#reporting}
   + 關於報告 {#about-reporting}
      + [關於動態報表](reporting/using/about-dynamic-reports.md)
      + [報告介面](reporting/using/reporting-interface.md)
      + [元件清單 ](reporting/using/list-of-components-.md)
      + [指標計算](reporting/using/indicator-calculation.md)
      + [疑難排解](reporting/using/troubleshooting.md)
   + 報表清單 {#list-of-reports}
      + [定義報告時段](reporting/using/defining-the-report-period.md)
      + [反彈摘要](reporting/using/bounce-summary.md)
      + [依網域劃分](reporting/using/breakdown-by-domains.md)
      + [抱怨](reporting/using/complaints.md)
      + [傳送摘要](reporting/using/delivery-summary.md)
      + [傳送總處理能力](reporting/using/delivery-throughput.md)
      + [應用程式內報告](reporting/using/in-app-report.md)
      + [非交付項目和彈回數](reporting/using/non-deliverables-and-bounces.md)
      + [開啓次數](reporting/using/opens.md)
      + [推播通知報表](reporting/using/push-notification-report.md)
      + [SMS報告](reporting/using/sms-report.md)
      + [追蹤指標](reporting/using/tracking-indicators.md)
      + [取消訂閱](reporting/using/unsubscriptions.md)
      + [URL和點按流](reporting/using/urls-and-click-streams.md)
      + [熱門點按](reporting/using/hot-clicks.md)
   + 自訂報表 {#customizing-reports}
      + [新增面板](reporting/using/adding-panels.md)
      + [新增視覺效果](reporting/using/adding-visualizations.md)
      + [新增元件](reporting/using/adding-components.md)
      + [建立自訂描述檔維度](reporting/using/creating-a-custom-profile-dimension.md)
      + [根據工作流程區段建立報表](reporting/using/creating-a-report-workflow-segment.md)
+ 與Adobe Cloud整合 {#integrating-with-adobe-cloud}
   + [關於促銷活動整合](integrating/using/about-campaign-integrations.md)
   + 使用Campaign和Experience Manager {#working-with-campaign-and-experience-manager}
      + [與Experience Manager整合](integrating/using/integrating-with-experience-manager.md)
      + [在Experience Manager中建立促銷活動表單 ](integrating/using/creating-a-campaign-form-in-experience-manager-.md)
   + 使用促銷活動和目標 {#working-with-campaign-and-target}
      + [關於Campaign-Target整合](integrating/using/about-campaign-target-integration.md)
      + [設定促銷活動-目標整合](integrating/using/configuring-the-campaign-target-integration.md)
      + [新增Target動態內容](integrating/using/adding-target-dynamic-content.md)
   + 使用促銷活動與分析 {#working-with-campaign-and-analytics}
      + [關於Campaign-Analytics整合](integrating/using/about-campaign-analytics-integration.md)
      + [設定Campaign-Analytics整合](integrating/using/configure-campaign-analytics-integration.md)
      + [Analytics中的促銷活動維度和度量](integrating/using/campaign-dimensions-and-metrics-in-analytics.md)
   + 使用Campaign和Audience Manager或People核心服務 {#working-with-campaign-and-audience-manager-or-people-core-service}
      + [關於Campaign-Audience Manager或人員核心服務整合](integrating/using/about-campaign-audience-manager-or-people-core-service-integration.md)
      + [提供並設定與Audience Manager或People核心服務的整合](integrating/using/provisioning-and-configuring-integration-with-audience-manager-or-people-core-service.md)
      + [疑難排解](integrating/using/troubleshooting.md)
      + [與Audience Manager或People核心服務共用觀眾](integrating/using/sharing-audiences-with-audience-manager-or-people-core-service.md)
   + [使用促銷活動與資產核心服務](integrating/using/working-with-campaign-and-assets-core-service.md)
   + 使用Campaign和Analytics for Mobile {#working-with-campaign-and-analytics-for-mobile}
      + [關於促銷活動興趣點資料整合](integrating/using/about-campaign-points-of-interest-data-integration.md)
      + [設定促銷活動興趣點資料整合](integrating/using/configuring-campaign-points-of-interest-data-integration.md)
      + [使用興趣點資料個人化促銷活動訊息](integrating/using/personalizing-campaign-messages-with-point-of-interest-data.md)
   + 使用促銷活動和觸發器 {#working-with-campaign-and-triggers}
      + [關於Adobe Experience Cloud觸發器](integrating/using/about-adobe-experience-cloud-triggers.md)
      + [在Experience Cloud中設定觸發器](integrating/using/configuring-triggers-in-experience-cloud.md)
      + [在促銷活動中使用觸發器](integrating/using/using-triggers-in-campaign.md)
      + [測試觸發器](integrating/using/testing-your-triggers.md)
      + [放棄觸發器使用個案](integrating/using/abandonment-triggers-use-cases.md)
+ 管理程序和資料 {#managing-processes-and-data}
   + 關於工作流程與資料管理 {#about-workflows-and-data-management}
      + [工作流程資料與流程](automating/using/workflow-data-and-processes.md)
      + [探索工作流程](automating/using/discovering-workflows.md)
      + [工作流程作業原則](automating/using/workflow-operating-principles.md)
      + [工作流程介面](automating/using/workflow-interface.md)
   + 工作流程一般作業 {#workflow-general-operation}
      + [建立工作流程](automating/using/building-a-workflow.md)
      + [定位資料](automating/using/targeting-data.md)
      + [匯入資料](automating/using/importing-data.md)
      + [使用工作流程資料](automating/using/using-workflow-data.md)
      + [執行工作流程](automating/using/executing-a-workflow.md)
      + [使用外部參數來呼叫工作流程](automating/using/calling-a-workflow-with-external-parameters.md)
      + [工作流程最佳實務](automating/using/best-practices-workflows.md)
   + 定位活動 {#targeting-activities}
      + [關於定位活動](automating/using/about-targeting-activities.md)
      + [Query](automating/using/query.md)
      + [遞增查詢](automating/using/incremental-query.md)
      + [Union](automating/using/union.md)
      + [十字路口](automating/using/intersection.md)
      + [排除](automating/using/exclusion.md)
      + [分段](automating/using/segmentation.md)
      + [閱讀讀者群](automating/using/read-audience.md)
      + [儲存觀眾](automating/using/save-audience.md)
      + [去重復化](automating/using/deduplication.md)
      + [豐富型](automating/using/enrichment.md)
   + 執行活動 {#execution-activities}
      + [關於執行活動](automating/using/about-execution-activities.md)
      + [開始和結束](automating/using/start-and-end.md)
      + [分叉](automating/using/fork.md)
      + [AND-complement](automating/using/and-join.md)
      + [排程器](automating/using/scheduler.md)
      + [等候等](automating/using/wait.md)
      + [外部訊號](automating/using/external-signal.md)
      + [測試](automating/using/test.md)
   + 頻道活動 {#channel-activities}
      + [關於頻道活動](automating/using/about-channel-activities.md)
      + [電子郵件傳送](automating/using/email-delivery.md)
      + [SMS傳送](automating/using/sms-delivery.md)
      + [推播通知傳送](automating/using/push-notification-delivery.md)
      + [直接傳送郵件](automating/using/direct-mail-delivery.md)
      + [應用程式內散髮](automating/using/in-app-delivery.md)
   + 資料管理活動 {#data-management-activities}
      + [關於資料管理活動](automating/using/about-data-management-activities.md)
      + [更新資料](automating/using/update-data.md)
      + [載入檔案](automating/using/load-file.md)
      + [傳輸檔案](automating/using/transfer-file.md)
      + [調解](automating/using/reconciliation.md)
      + [摘取檔案](automating/using/extract-file.md)
      + [訂閱服務](automating/using/subscription-services.md)
      + [外部API](automating/using/external-api.md)
   + 篩選資料 {#filtering-data}
      + [編輯查詢](automating/using/editing-queries.md)
      + [進階運算式編輯](automating/using/advanced-expression-editing.md)
      + [函數清單](automating/using/list-of-functions.md)
   + 匯入和匯出資料 {#importing-and-exporting-data}
      + [關於資料匯入與匯出](automating/using/about-data-import-and-export.md)
      + [使用匯入範本匯入資料](automating/using/importing-data-with-import-templates.md)
      + [定義匯入範本](automating/using/defining-import-templates.md)
      + [匯出記錄檔](automating/using/exporting-logs.md)
      + [管理套件](automating/using/managing-packages.md)
      + [匯出清單](automating/using/exporting-lists.md)
   + 使用案例：工作流程 {#workflow-use-case}
      + [建立具有輔助功能的傳送](automating/using/workflow-created-query-with-complement.md)
      + [建立建立日期時的傳送](automating/using/workflow-creation-date-query.md)
      + [建立每周傳送](automating/using/workflow-weekly-offer.md)
      + [在位置建立區段](automating/using/workflow-segmentation-location.md)
      + [重新定位非\ openers](automating/using/workflow-cross-channel-retargeting.md)
+ 開發 {#developing}
   + 關於自訂資源 {#about-custom-resources}
      + [資料模型概念](developing/using/data-model-concepts.md)
      + [資源狀態](developing/using/resource-statuses.md)
      + [監控資料模型變更](developing/using/monitoring-data-model-changes.md)
   + 新增或擴充資源 {#adding-or-extending-a-resource}
      + [新增資源的關鍵步驟](developing/using/key-steps-to-add-a-resource.md)
      + [建立或擴充資源](developing/using/creating-or-extending-the-resource.md)
      + [設定資源的資料結構](developing/using/configuring-the-resource-s-data-structure.md)
      + [設定螢幕定義](developing/using/configuring-the-screen-definition.md)
      + [設定篩選定義](developing/using/configuring-filter-definition.md)
      + [更新資料庫結構](developing/using/updating-the-database-structure.md)
      + [刪除資源](developing/using/deleting-a-resource.md)
      + [使用復合識別金鑰來呼叫資源](developing/using/uc-calling-resource-id-key.md)
   + 使用案例：擴充API {#use-case--extending-the-api}
      + [關於延伸API](developing/using/about-extending-the-api.md)
      + [步驟1：新增副檔名至描述檔資源](developing/using/step-1--add-extension-fields-to-the-profile-resource.md)
      + [步驟2：發佈擴充功能](developing/using/step-2--publish-the-extension.md)
      + [步驟3：驗證擴充功能](developing/using/step-3--verify-the-extension.md)
   + 使用案例：擴充資源 {#use-cases--extending-resources}
      + [使用新欄位擴充描述檔資源](developing/using/extending-the-profile-resource-with-a-new-field.md)
      + [將訂閱延伸至應用程式資源](developing/using/extending-the-subscriptions-to-an-application-resource.md)
+ 管理 {#administrating}
   + [關於管理Adobe Campaign](administration/using/about-administrating-adobe-campaign.md)
   + 使用者與安全性 {#users-and-security}
      + [關於存取管理](administration/using/about-access-management.md)
      + [使用者管理](administration/using/users-management.md)
      + [組織單位](administration/using/organizational-units.md)
      + [角色清單](administration/using/list-of-roles.md)
      + [管理群組和使用者](administration/using/managing-groups-and-users.md)
      + [稽核匯出記錄檔](administration/using/auditing-export-logs.md)
      + [授權](administration/using/licenses.md)
   + 設定渠道 {#configuring-channels}
      + [關於頻道設定](administration/using/about-channel-configuration.md)
      + [設定電子郵件頻道](administration/using/configuring-email-channel.md)
      + [設定SMS頻道](administration/using/configuring-sms-channel.md)
      + [設定行動應用程式](administration/using/configuring-a-mobile-application.md)
      + [設定交易式傳訊](administration/using/configuring-transactional-messaging.md)
   + 使用打字規則 {#working-with-typology-rules}
      + [關於打字規則](administration/using/about-typology-rules.md)
      + [篩選規則](administration/using/filtering-rules.md)
      + [疲勞規則](administration/using/fatigue-rules.md)
      + [控制規則](administration/using/control-rules.md)
   + 應用程式設定 {#application-settings}
      + [外部帳戶](administration/using/external-accounts.md)
      + [促銷活動中的定位對應](administration/using/target-mappings-in-campaign.md)
      + [技術工作流程](administration/using/technical-workflows.md)
      + [品牌推廣](administration/using/branding.md)
      + [傳送內部通知](administration/using/sending-internal-notifications.md)
      + [資料保留](administration/using/data-retention.md)
      + [更新與維護作業](administration/using/updates-and-maintenance-operations.md)
