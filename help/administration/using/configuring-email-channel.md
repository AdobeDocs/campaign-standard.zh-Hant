...
* **授權遮罩欄位**
  ...
  **[!UICONTROL Header parameters of sent emails]**&#x200B;區段列出您可用於傳送電子郵件給收件者（寄件者地址），並讓他們傳回自動回覆的電子郵件地址，例如非同步退件、休假回覆等（錯誤地址）。
...
* **傳送參數**
  ...
  >[!IMPORTANT]
  >
  >**現在，Campaign中只會在設為3.5天或更少時使用。** 如果您定義的值超過3.5天，則不會考慮該值。
...
* **[!UICONTROL Delivery duration]** / **[!UICONTROL Validity limit for sending messages]**：Adobe Campaign　會從開始日期開始傳送訊息。 使用此欄位來指定可傳送訊息的期間。
  ...
  >[!IMPORTANT]
  >
  >**您必須定義最多3.5天的值。** 如果您設定的值超過3.5天，則不會考慮該值。
  >
...
* **[!UICONTROL Typology]**：在任何傳送作業之前，必須準備郵件，才能驗證內容和設定。 在&#x200B;**類型**&#x200B;中定義準備階段期間，會套用驗證規則。 例如，對於電子郵件，準備涉及檢查主題、URL和影像等。選取要套用在此欄位中的型別。
...
* **[!UICONTROL Additional SMTP headers]**：此選項可將其他　SMTP　標頭新增至您的郵件中。 在&#x200B;**[!UICONTROL Headers]**&#x200B;欄位中輸入的指令碼必須參考每行一個標頭，其形式為&#x200B;**名稱:value**。 如有必要，會自動對值編碼。
  ...
  Google 和 Yahoo! 將於 2024 年 6 月 1 日起 要求寄件者遵守&#x200B;**一鍵式清單取消訂閱**。 Campaign可針對傳遞範本支援此現成可用的功能。
