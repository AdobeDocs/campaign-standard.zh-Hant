...
description: 了解如何設定標籤規則以支援Adobe Campaign Standard使用案例
audience: channels
...
在資料收集UI中，建立資料元素和規則，將PII和其他資料從行動應用程式傳送至 [!DNL Adobe Campaign Standard].

若要確保資料收集UI中的所有設定變更生效，您必須發佈這些變更。 如需詳細資訊，請參閱 [發佈](https://developer.adobe.com/client-sdks/documentation/getting-started/create-a-mobile-property/#publish-the-configuration).

若要在資料收集UI中建立規則，請執行下列步驟：
...
1. 在資料收集UI中，從行動應用程式控制面板按一下 **[!UICONTROL Data Elements]** 標籤。
...
將PII資料傳送至 [!DNL Adobe Campaign Standard]，請在資料收集UI中建立規則：

1. 在資料收集UI中，從行動應用程式控制面板按一下 **[!UICONTROL Rules]** 然後 **[!UICONTROL Create New Rule]**.
...
1. 在 **[!UICONTROL Post Body]**，請輸入下列內容：
   ...
   資料收集UI中定義的資料元素應以雙重百分比括住，例如 `%%mcid%%`，以及來自應用程式的內容變數應以單一百分比括住，例如%contextdata.email%。
...
若要將追蹤資料傳送至 [!DNL Adobe Campaign Standard] 若要報告使用者在行動應用程式中與應用程式內訊息互動的方式，請在資料收集UI中建立下列規則：

1. 在資料收集UI中，從行動應用程式控制面板中，選取 **[!UICONTROL Rules]** 按一下 **[!UICONTROL Add Rule]**.
...
若要將追蹤資料傳送至 [!DNL Adobe Campaign Standard]，有助於追蹤推播通知傳送以及使用者與行動應用程式的互動，您必須在資料收集UI中建立規則。
...
1. 在資料收集UI中，從行動應用程式控制面板按一下 **[!UICONTROL Rules]** 按一下 **[!UICONTROL Add Rule]**.
...
1. 選取 **[!UICONTROL Add post body]** 核取方塊並新增貼文內文，例如：
   ...
   >[!NOTE]
   >
   >在上述範例中，您必須善用 [建立資料元素](../../administration/using/configuring-rules-launch.md#create-data-elements). 支援左側的資料元素 [!DNL Adobe Campaign Standard] 且不需要任何設定。 若需要其他資料，您必須在 [!DNL Adobe Campaign Standard].
