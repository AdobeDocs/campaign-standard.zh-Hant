...
* 檢查Adobe Campaign是否及時回覆成功的`DELIVER_SM_RESP PDU`。 在Adobe Campaign Standard上，這可保證已套用整個處理邏輯，如果不是這種情況，可保證在記錄中有錯誤訊息，說明處理失敗的原因。

如果`DELIVER_SM PDU`未成功確認，則您應該檢查下列專案：
