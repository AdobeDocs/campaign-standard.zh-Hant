...
此使用情形說明了如何使用與目標維不同的資源來查找遠程表中的特定記錄。

有關目標維和資源的詳細資訊，請參閱 [此部分](../../automating/using/query.md#targeting-dimensions-and-resources)

**範例　1：使用標籤「歡迎回來！」識別傳送所定位的設定檔**。

* 在此案例中，我們要定位設定檔。我們會將目標維度設為　**[!UICONTROL Profiles (profile)]**。
* 我們想要根據傳送標籤來篩選所選取的設定檔。因此，我們將資源設定為　**[!UICONTROL Delivery logs]**。這樣，我們直接在傳遞日誌表中進行篩選，這將提供更好的效能。
