...
**運用Campaign訊息控制面板** 從單一位置配置和發送您的郵件，並訪問各種功能，如計畫、多語言消息……

**管理範本** 配置消息屬性，並允許您獲得消息策略的時間和一致性。 例如，對於電子郵件，負責設定範本的功能管理員可以定義電子郵件的標準參數，例如對象、排程或內容、與傳送、有效性、追蹤等相關的進階參數，或關於鎖定目標和個人化（鎖定目標內容）的預先設定參數。
...
**5個通信通道** 可在Adobe Campaign中使用：電子郵件、SMS訊息、推播通知、應用程式內訊息和直接郵件傳送。

對於每個管道，都可使用特定功能。 例如，電子郵件通道可讓您執行A/B測試，以測試最多3個版本的電子郵件訊息。 透過SMS通道，管理停止SMS訊息，並將來自收件者的傳入SMS儲存至您的資料庫。

此外，還要利用Campaign Standard **個人化和動態內容功能** 來擷取觀眾的興趣。
...
**傳送個別和不重複訊息** 即時傳送給客戶：歡迎郵件、訂單運送確認、密碼修改等。

交易式訊息可用於電子郵件、簡訊和推播通知通道，視您的選項而定。 訊息有兩種類型： **事件交易式訊息** 定位不含設定檔資訊的事件，以及 **設定檔交易式訊息** 從資料庫定位設定檔。
...
Campaign隨附著登錄頁面，這些網頁是可用於 **擷取您對象的資訊**，提供服務的訂閱、顯示資料及擴充資料庫。

登錄頁面也可用於 **取得或更新現有設定檔**，以及設定雙重加入機制，讓您保護平台，使其免受錯誤或無效的電子郵件地址或垃圾郵件機器人的侵擾。
