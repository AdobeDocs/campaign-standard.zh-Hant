...
## 事務性消息傳遞定義{#transactional-messaging-definition}
...
* 它們與[疲勞規則](../../sending/using/fatigue-rules.md)不相容（即使在具有輪廓的富集情況下）。
...
* 請注意，此步驟必須由具有[管理權限](../../administration/using/users-management.md#functional-administrators)的用戶執行。
...
* 此步驟可由具有[標準使用者存取權限](../../administration/using/users-management.md#basic-users)的任何行銷使用者執行。

有關編輯和發佈事務性消息的詳細資訊，請參閱[事件事務性消息](../../channels/using/event-transactional-messages.md)。
...
如需將事件整合至網站的詳細資訊，請參閱[網站整合](../../administration/using/configuring-transactional-messaging.md#integrating-the-triggering-of-the-event-in-a-website)。
