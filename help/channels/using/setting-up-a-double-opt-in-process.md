...
1. 建立並發佈登錄頁面，讓訪客可以註冊並訂閱。此登錄頁面可從網站取得。填寫並提交此登錄頁的訪問者將儲存在資料庫中，但會添加到登錄清單中，以便在最終驗證之前不接收任何通信(請參閱 [市場活動中的Denylist管理](../../audiences/using/about-opt-in-and-opt-out-in-campaign.md))。
...
1. 在　**[!UICONTROL Job]** > **[!UICONTROL Additional data]**　區段中，按一下　**[!UICONTROL Add an element]**　並輸入下列內容路徑：
   ...
   此上下文將刪除「On denylist」欄位，以便能夠發送電子郵件。 我們稍後會看到，第一個登錄頁面在確認前將此欄位設為 **true**，以防止傳送電子郵件至未確認的設定檔。如需詳細資訊，請參閱 [步驟 3：建立贏取登錄頁面](#step-3--create-the-acquisition-landing-page)。
...
您需要建立特定[類型](../../sending/using/about-typology-rules.md)，方法是複製現成可用的類型。分類將允許向尚未確認其協定且仍在密文清單中的配置檔案發送消息。 預設情況下，類型排除opt-out（即denylist）配置式。 若要建立此類型，請依照下列步驟進行：
...
1. 編輯登錄頁面屬性：在&#x200B;**[!UICONTROL Job]** > **[!UICONTROL Additional data]** 區段中，按一下 **[!UICONTROL Add an element]** 並輸入下列內容路徑：
   ...
   這是強制添加密文清單和避免向未確認其協定的訪問者發送消息的強制要求。 驗證 CONFIRMATION 登錄頁面後，會在確認後將此欄位設 為 **false**。如需詳細資訊，請參閱[步驟 1：建立確認登錄頁面](#step-1--create-the-confirmation-landing-page)。
