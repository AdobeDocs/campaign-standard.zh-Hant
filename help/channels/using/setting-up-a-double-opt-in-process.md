...
1. 建立並發佈登錄頁面，讓訪客可以註冊並訂閱。此登錄頁面可從網站取得。填寫並提交此登錄頁面的訪客會儲存在資料庫中，但會新增至封鎖清單，以便在最終驗證之前不會收到任何通訊(請參閱 [Campaign中的封鎖清單管理](../../audiences/using/about-opt-in-and-opt-out-in-campaign.md))。
