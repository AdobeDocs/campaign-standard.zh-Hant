...
例如，無法將事件指派給執行傳送，因為其內容不正確、存取許可權或品牌推廣有問題、套用型別規則時偵測到錯誤等。在此情況下，您可以暫停訊息、編輯訊息以修正問題，然後再次發佈。 之後，重試系統會將其指派給新的執行傳送。
...
選取 **[!UICONTROL Sending logs]** 索引標籤。 在 **[!UICONTROL Status]** 欄中，**[!UICONTROL Sent]** 表示輪廓已選取加入。
...
對於已選取退出的任何輪廓，**[!UICONTROL Address on denylist]**　類型規則會排除相對應的收件人。
