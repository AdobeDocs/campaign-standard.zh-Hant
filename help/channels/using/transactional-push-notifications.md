...
product: campaign
title: 異動推播通知
description: 瞭解如何與Adobe Campaign Standard傳送交易推播通知。
audience: channels
...
您可以使用Adobe Campaign來傳送iOS和Android行動裝置上的交易推播通知。 這些訊息是在您在Adobe Campaign設定的行動應用程式上，透過運用Experience Cloud行動SDK來接收。
...
* [針對Adobe Campaign資料庫的](#transactional-push-notifications-targeting-a-profile) 交易推播通知設定檔

## 針對事件{#transactional-push-notifications-targeting-an-event}的交易式推播通知

您可以使用Adobe Campaign來傳送&#x200B;**匿名交易推播通知給已選擇從行動應用程式接收通知的所有使用者。**

在這種情況下，僅使用事件本身包含的&#x200B;**資料來定義傳送目標**。 沒有利用Adobe Campaign綜合配置檔案資料庫的資料。
...
* **註冊Token**，此為一個行動應用程式與一個裝置的使用者ID。 它可能不對應於Adobe Campaign資料庫中的任何概要檔案。
* **行動應用程式名稱**（所有裝置皆有一個- Android和iOS）。 這是在Adobe Campaign設定的行動應用程式ID，將用來接收使用者裝置上的推播通知。 有關詳細資訊，請參閱[配置移動應用程式](../../administration/using/configuring-a-mobile-application.md)。
...
1. 如果您想使用Adobe Campaign資料庫的其他資訊，也可以豐富事務性消息內容（請參閱[豐富事件](../../channels/using/configuring-transactional-event.md#enriching-the-transactional-message-content)）。

1. 儲存您的變更並發佈訊息。請參閱[發佈交易式訊息](../../channels/using/publishing-transactional-message.md#publishing-a-transactional-message)。

1. 使用Adobe Campaign StandardREST API，在Android(gcm)上使用一個行動應用程式(WeFlight)，將事件傳送至註冊Token(ABCDEF123456789)，其中包含登入資料：
...
您可以傳送交易推播通知&#x200B;**給已訂閱您行動應用程式**&#x200B;的Adobe Campaign設定檔。 此傳送可包含[個人化欄位](../../designing/using/personalization.md#inserting-a-personalization-field)，例如從Adobe Campaign資料庫直接擷取的收件者名字。

在這種情況下，事件必須包含一些欄位&#x200B;**，允許與來自Adobe Campaign資料庫的概要檔案進行協調。**

定位設定檔時，會依行動應用程式和裝置傳送一個交易式推播通知。 例如，如果Adobe Campaign用戶訂閱了兩個應用程式，則該用戶將收到兩個通知。 如果使用者已訂閱使用兩種不同裝置的相同應用程式，此使用者會在每種裝置上收到通知。
...
若要傳送交易推播通知給已訂閱您行動應用程式的Adobe Campaign個人檔案，您首先需要建立並設定以Adobe Campaign資料庫為目標的事件。
...
1. 如果要個性化事務性消息，請向事件添加欄位（請參閱[定義事件屬性](../../channels/using/configuring-transactional-event.md#defining-the-event-attributes)）。
   ...
   >[!NOTE]
   >
   >您必須至少添加一個欄位才能建立富集。 您不需要建立其他欄位，例如&#x200B;**名字**&#x200B;和&#x200B;**姓氏**，因為您將能夠使用Adobe Campaign資料庫中的個人化欄位。
...
例如，一家航空公司想要向所有訂閱其行動應用程式的Adobe Campaign用戶發出上線通知。
...
1. 儲存您的變更並發佈訊息。請參閱[發佈交易式訊息](../../channels/using/publishing-transactional-message.md#publishing-a-transactional-message)。
1. 使用Adobe Campaign StandardREST API，將事件傳送至描述檔：
...
相應用戶接收包括從Adobe Campaign資料庫檢索的所有個人化元素的事務性推播通知。
