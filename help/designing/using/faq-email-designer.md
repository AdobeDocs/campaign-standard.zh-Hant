...
* 內容區塊是純粹的HTML，手動插入HTML程式碼（不方便使用的UI，而是直接原始程式碼）。 雖然此範本的真正目標是供具有HTML知識的人使用，但可允許內容片段中不提供的個人化層級。
...
您可以將自己的HTML內容上傳至電子郵件Designer。 如果尚未透過電子郵件Designer建立，則會以相容性模式載入，這種模式旨在保留您原始的HTML，但透過UI限制某些版本功能。
