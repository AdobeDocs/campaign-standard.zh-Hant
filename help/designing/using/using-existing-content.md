...
## 使用電子郵件Designer編輯現有內容{#editing-existing-contents-with-the-email-designer}
...
zip檔案結構沒有限制。 不過，參考HTML檔案必須是相對的，並且遵守zip資料夾的樹狀結構。
...
1. URL內容在編輯器中顯示為唯讀。
   ...
   >[!CAUTION]
   >
   >在此步驟中，不應考量內容編輯器中的HTML顯示。 它將在準備階段擷取。
...
如果上傳的HTML全部或部分不符合預期的標籤，內容則會在「相容模式」中載入，這會限制透過UI編輯的可能性。
...
您必須透過HTML模式，直接在電子郵件原始碼中完成其他修改，例如將新區段新增至您的電子郵件或進階樣式。
...
在電子郵件Designer之外，請確定原始HTML分成多個可重複使用的區段。

如果不是這種情況，請從HTML中剪下不同的區塊。 例如：
