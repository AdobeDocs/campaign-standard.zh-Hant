...
# 使用新欄位擴充輪廓資源{#extending-the-profile-resource-with-a-new-field}

## 關於擴充設定檔 {#about-extending-profiles}

此使用案例詳細說明如何使用專用欄位擴充輪廓和測試輪廓。

在此處，我們想使用登錄頁面，以新欄位更新我們的輪廓，然後以符合其興趣的電子報來選擇目標輪廓。
...
若要為輪廓建立新的&#x200B;**興趣**&#x200B;欄位，您必須先擴充現成的 **[!UICONTROL Profiles (profile)]** 資源。
...
1. 若要新增分隔符號，以便更妥善地組織您的輪廓視窗，請按一下 **[!UICONTROL Create an element]** 並從 **[!UICONTROL Type]** 下拉式清單中選取 **[!UICONTROL Separator]**。

   ![](assets/schema_extension_uc19.png)

您的欄位現已設定完畢。 現在，我們需要將它擴充至測試輪廓。

>[!NOTE]
>
>如果您不需要擴充測試輪廓資源，可跳至「發佈」步驟。

## 步驟2：擴充測試設定檔 {#step-2--extend-the-test-profile}

若要測試新建立的欄位是否已正確設定，您可以將傳遞傳送至測試輪廓，藉以測試欄位。 首先，需要對測試輪廓執行新領域。
...
1. 從步驟 11 到 13 執行與上述延伸輪廓步驟相同的步驟，將此欄位新增至 **[!UICONTROL Test profile]** 畫面中。
1. 按一下 **[!UICONTROL Save]**。

現在，輪廓和測試輪廓都可使用您的新欄位。 若要正確設定，您必須發佈自訂資源。
...
若要使用新自訂欄位的資料更新輪廓，您可以使用 **[!UICONTROL Profile acquisition]** 範本建立登錄頁面。 如需登錄頁面的詳細資訊，請參閱[本區段](../../channels/using/getting-started-with-landing-pages.md)。

在此處，我們想要鎖定未填入此欄位的工作流程輪廓。 他們會收到電子郵件，要求他們更新輪廓，以接收個人化的電子報和產品建議。 然後，每個輪廓都會收到個人化電子報，內容視其選取的興趣而定。

首先，我們需要建立登錄頁面，以更新目標輪廓&#x200B;**興趣**&#x200B;欄位：

1. 從 **[!UICONTROL Marketing activities]** 按一下 **[!UICONTROL Create]**，然後選取 **[!UICONTROL Landing page]**。
1. 選取登錄頁面類型。 因為我們要更新輪廓，請選取 **[!UICONTROL Profile acquisition]**。
1. 按一下 **[!UICONTROL Create]**。
...
1. 視需要自訂您的登錄頁面。
1. 按一下針對您的輪廓所設定的欄位，以便在興趣之間做出選取。 在左窗格中，選取您先前建立的&#x200B;**興趣**&#x200B;自訂資源。
...
1. 新增 **[!UICONTROL Wait]** 活動，讓您的輪廓有時間填入登錄頁面。
...
輪廓現在會收到電子郵件，要求他們填寫此「興趣」欄位，之後會根據所選的值傳送個人化電子郵件。
