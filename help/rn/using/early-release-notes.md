...
<ul>
<li>會擷取所有事件類別：延遲、傳送至傳送、取消訂閱（連結、清單）、回饋（垃圾訊息投訴、非同步事件）。</li>
...
<tr> 
<td>
<p>與Adobe Experience Manager的整合已改善：您現在可以更輕鬆地從Adobe Experience Manager匯入多語言內容。 Adobe Campaign Standard現在會自動從Adobe Experience Manager內容中偵測語言變體，並允許大量匯入和建立變體，大幅簡化從業人員根據Adobe Experience Manager內容建立多語言促銷活動所需執行的步驟。
</p>
</td> 
</tr> 
</tbody> 
...
* Microsoft Dynamics 365整合已增強，提供專屬的自助服務整合應用程式和改良的實施程式。 [進一步瞭解](../../integrating/using/d365-acs-get-started.md)
...
* 「配置式」清單現在允許您根據以下欄位之一搜索記錄：擴充描述檔資源時，已在進階篩選中新增的電子郵件、名字、姓氏或自訂欄位。 此功能也可在使用filterType參數的Campaign Standard API中使用。

* 參數已調整為執行「事務性傳訊」資料庫集區程式的容器數。 這可讓負載均勻分佈於所有使用的容器，並達到最佳效能。

* 在使用事件變數呼叫具有外部參數的工作流程後，活動現在可使用新函式(GetOption)。 它允許您返回指定函式的值。

* 已新增新的技術選項。 它可讓Campaign Standard在啟動工作流程之前，先檢查系統上是否有足夠的實體記憶體。 如果記憶體量太低，工作流執行將延遲，直到系統記憶體達到此閾值。 這樣做是為了避免效能進一步降低並降低停機風險。 請嘗試將此工作流程重新排程為活動較少的時間，然後再試一次。 當伺服器壓力放鬆後，工作流程就會自動恢復。 請注意，此選項為唯讀，無法修改。

**其他變更**

* 將訊息準備期間的錯誤變更為警告，當達到每個滾動小時100個內容下載的限制時。 現在，當達到限制時會顯示警告，允許繼續傳送。

* 新的傳送對應 (mapRtEventAppSubRcp) 現在可用於定位設定檔的交易式性推送訊息。這些傳送的傳遞、排除和追蹤記錄現在可在 broadLogAppSubRcp、excludeLogAppSubRcp 和 trackingLogAppSubRcp 資料表中使用。這解決了使用設定檔目標維度發送交易式推送訊息時，所導致的傳送分析失敗的問題。
...
* SMS連接器現在可以隨每則訊息傳送多個可選參數。

* 修正DATA MODEL角色的使用者無法發佈傳送記錄副檔名的問題。 此操作現在可用於DATA MODEL角色。 (CAMP-46604)
...
* 下列語言已新增至「偏好的語言」清單：印度尼西亞——印度尼西亞(in-id)、英文——瑞典(en-se)、英文——亞太(en-ap)、英文——日本(en-jp)、西班牙——拉丁美洲(es-la)。 (CAMP-46351)
...
* 您現在可以使用動態報表中的搜尋列進一步篩選「傳送」和「促銷活動」維度。
