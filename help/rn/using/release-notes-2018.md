...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 應用程式內傳訊功能可讓您提供情境式互動，並觸及停用推播通知的使用者，更有效與行動應用程式使用者互動。 搭配推播通知使用應用程式內傳訊，建立高度個人化和相關的體驗。 這可讓您的應用程式使用者獲得更好的轉換和保留。<br /> 如需詳細資訊，請參閱<a href="../../channels/using/about-in-app-messaging.md">詳細檔案</a>.<br /> </td> 
  </tr> 
  <tr> 
   <td> 適用於行動應用程式的Adobe Launch整合（測試版）<br /> </td> 
   <td> Adobe Launch與Adobe Campaign的整合現在採用Mobile SDK V5.<br />，簡化並自動化Campaign的行動應用程式啟用程式。 如需詳細資訊，請參閱<a href="https://helpx.adobe.com/tw/campaign/kb/configuring-app-sdk.html">詳細檔案</a>.<br /> </td> 
  </tr> 
 </tbody> 
</table>
...
* 在Campaign中設定行動應用程式時，使用者現在可以知道iOS憑證和Android伺服器金鑰是否已成功上傳及其到期日。
  ...
  如需詳細資訊，請參閱有關如何使用[SDK V4](https://helpx.adobe.com/tw/campaign/kb/configuring-app-sdkv4.html)和[SDK V5](https://helpx.adobe.com/tw/campaign/kb/configuring-app-sdk.html)設定行動應用程式的詳細檔案。

* 在定義促銷活動屬性時，藉由選取行動應用程式來鎖定特定行動應用程式上的使用者。 此功能同時適用於推送和應用程式內傳訊頻道。

  如需詳細資訊，請參閱[詳細說明文件](../../channels/using/preparing-and-sending-a-push-notification.md#preparing-the-notification)。
...
* 修正使用Internet Explorer 11時，拖放功能無法在Creative Designer中正確運作的問題。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 現在，您可以在準備階段從URL匯入電子郵件內容。 對於循環電子郵件傳遞，每當準備訊息時會擷取最新的HTML內容，以確保在傳送電子郵件時內容永遠為最新。 此功能也可讓您從URL建立包含內容的排程傳遞，即使內容尚未準備就緒。<br /> 如需詳細資訊，請參閱<a href="../../designing/using/using-existing-content.md#retrieving-content-from-a-url-automatically-at-preparation-time">詳細檔案</a>.<br /> </td> 
  </tr> 
  <tr> 
   <td> 行銷活動發行通知訊息<br /> </td> 
  ...
  <tr> 
   <td> 使用者管理<br /> </td> 
   <td> 從18.7版開始，新的Campaign Standard執行個體以及未建立地理單位的現有執行個體現在無法使用地理單位功能。<br /> 如需詳細資訊，請參閱此<a href="https://experienceleague.adobe.com/docs/campaign-standard/using/release-notes/deprecated-features.html?lang=zh-Hant#release-notes">頁面</a>.<br /> </td> 
  </tr> 
 </tbody> 
</table>
...
* 編輯電子郵件內容時，新的Creative Designer預設為開啟。 如果願意，您仍然可以在儲存變更後隨時返回預設內容編輯器。 如需詳細資訊，請參閱[詳細檔案](../../designing/using/designing-content-in-adobe-campaign.md)。
...
* 在交易式訊息熱點按報表中，當您按一下&#x200B;**變更設定檔**&#x200B;按鈕時，現在只會列出連結至您為交易式訊息定義之事件的測試設定檔。

**修補程式**

* 修正byEmail查詢篩選器無法傳回任何結果的問題。 (CAMP-23420)
* 修正允許標準使用者存取限制給管理員的特定功能或畫面（/rest/head/&#42;端點、異動訊息畫面、設定檔和對象匯入畫面）的問題。
* 修正GDPR隱私權刪除請求無法處理自訂資源（若名稱以數字開頭）的問題。
* 修正儲存對象活動無法在Adobe Experience Cloud中共用應用程式訂閱者的錯誤。
...
* **[!UICONTROL History]** API已新增至Adobe.IO。 它可讓您存取與設定檔行銷記錄相關的資訊：接觸點數目、已傳送內容、映象頁面URL等。如需詳細資訊，請參閱[專屬使用案例](../../api/using/interacting-with-marketing-history.md) 。
* 已最佳化&#x200B;**[!UICONTROL Database cleanup]**&#x200B;技術工作流程，以確保資料庫備份有更好的效能。
* 適用於電子郵件的Creative Designer現在也提供法文和德文版。

**其他變更**

* 已在已傳送傳遞的&#x200B;**[!UICONTROL Deployment]**&#x200B;視窗中新增&#x200B;**[!UICONTROL Compute stats]**&#x200B;按鈕。 它可讓您擷取最新的KPI，例如，如果傳送的結果更新時間過長或未考慮在內。 如需詳細資訊，請參閱本[區段](../../sending/using/confirming-the-send.md)。
* 在傳遞能力的&#x200B;**更新**&#x200B;現成可用的技術工作流程中，功能管理員現在可以在&#x200B;**更新規則** Javascript活動中定義要忽略的連續錯誤數。 預設情況下，欄位值設為0，這表示將忽略所有錯誤。
* 已最佳化管理單位存取限制條件時所產生的SQL。
* **[!UICONTROL Update]**&#x200B;活動現在可讓您新增、更新或刪除與訂閱（nms:appSubscriptionRcp資料表）相關的資料。
* 為了最佳化效能，**[!UICONTROL Update delivery execution]**&#x200B;技術工作流程已分成兩個工作流程： - **[!UICONTROL Update delivery execution]**：更新傳遞的追蹤。 預設會每10分鐘啟動一次。 **[!UICONTROL Update delivery indicators]**：更新傳遞的KPI，預設會每小時啟動一次。 如需技術工作流程的詳細資訊，請參閱此[區段](../../administration/using/technical-workflows.md#list-of-technical-workflows)。
* 當傳遞傳送訊息時，**[!UICONTROL Deployment]**&#x200B;區段中的狀態現在可以有兩個值： **[!UICONTROL Sending]**：正在傳送訊息。 **[!UICONTROL Sending (retry)]**：重試階段正在進行中。
* 具有&#x200B;**[!UICONTROL Delivery preparation]**&#x200B;角色的使用者現在可以傳送校樣。 (CAMP-24313)
* **透過SMPP啟用TLS**&#x200B;選項已新增到透過SMPP **外部帳戶的** SMS路由。 如需詳細資訊，請參閱此[區段](../../administration/using/configuring-sms-channel.md#defining-an-sms-routing)。

**修補程式**

* 修正從Adobe Target加入動態影像時，無法傳送電子郵件的問題(CAMP-24848)。
* 修正&#x200B;**[!UICONTROL Privacy Access/Delete Request]**&#x200B;技術工作流程的問題，若有任何要求失敗，這些工作流程將無法完成。
* 修正隱私權核心服務無法從Campaign接收請求狀態更新的問題。
* 修正了&#x200B;**[!UICONTROL Import shared audience]**&#x200B;技術工作流程無法正常運作的問題(CAMP-25465)。
* 修正在核心Privacy Service中無法將Campaign隱私權要求標示為已完成的問題。
* 修正了當Campaign Standard太長時，某些使用者無法透過IMS驗證登入Adobe ID的問題。 (CAMP-24095)
* 修正了Creative Designer中移除內容模組時可能發生的問題。 (CAMP-25242)
* 針對資料庫中無設定檔的訂閱者，修正使用推播通知疲勞規則的問題。 (CAMP-25344)
* 修正存取傳遞排除記錄檔時，可能顯示錯誤訊息的問題。 (CAMP-24724)
* 修正無法在具有延伸傳送記錄檔的執行個體中準備校樣的問題。
* 修正了在啟用&#x200B;**[!UICONTROL Sending log]**&#x200B;擴充功能的情況下發佈自訂資源時可能發生的兩個問題。
* 修正定期傳送未考慮傳送期間的情況下可能發生的問題。
* 針對擁有超過100,000筆記錄的自訂資源，修正在&#x200B;**[!UICONTROL Client data]**&#x200B;功能表中排序資料時可能發生的問題。 (CAMP-24308)
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> Adobe Campaign現在提供透過MCPNS接收來自提供者(APNS/GCM)之推播訊息的詳細意見反應（傳送記錄檔及排除記錄檔）的功能。<br /> 如需詳細資訊，請參閱<a href="../../channels/using/preparing-and-sending-a-push-notification.md#sending-the-notification">詳細檔案</a>.<br /> </td> 
  </tr> 
  <tr> 
   <td> 傳遞記錄延伸<br /> </td> 
   <td> 傳送記錄擴充功能可讓您使用來自工作流程的設定檔資料和區段程式碼來擴充傳送記錄檔。然後，此資訊便可用於動態報告中，並可讓您在傳送資料時保留某些資訊的快照。<br />還有2個使用案例：<br /> 
    <ul> 
     <li> 匯出包含「凍結」資料的擴充broadlog：作為行銷人員，我想匯出區段代碼等於「A」（來自工作流程引擎）的所有設定檔。 </li> 
     <li> 針對「凍結」資料細分：身為行銷人員，我希望<strong>重新鎖定自上次傳送以來已獲得1000點忠誠度或區段代碼等於「A」的所有設定檔</strong>。 </li> 
  ...
  <tr> 
   ...
   <td> 此功能可讓您根據在設定檔資源擴充期間建立的自訂設定檔資料，建立和管理報表。 您可以依個人資料屬性（例如熟客方案、偏好的管道等）劃分報表。<br />如需詳細資訊，請參閱<a href="../../developing/using/configuring-the-resource-s-data-structure.md#defining-sending-logs-extension">詳細檔案</a>。<br /> </td> 
  </tr> 
 </tbody> 
</table>
...
* 預設共用資料來源的識別碼現在處於唯讀模式，無法再變更。 變更此識別碼可能會導致與Experience Cloud共用受眾時發生一些問題。
...
* 「sourceId」欄位現在於追蹤記錄資源(nms:trackingLog)中公開。
...
* 修正可能無法載入Creative Designer的問題。 (CAMP-24573)
...
* 修正可能導致某些訊息未在&#x200B;**傳送**、**傳遞**&#x200B;和&#x200B;**退回**&#x200B;量度中納入考量的問題。
* 修正&#x200B;**傳遞摘要報告**&#x200B;中所選時間範圍太長的錯誤。
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> GDPR是歐盟(EU)的新隱私權法律，協調資料保護要求並以現代化方式規範這些要求，於2018年5月25日生效。GDPR適用於所持有資料的主體居住於歐盟的Adobe Campaign客戶。<br />除了Adobe Campaign中已提供的隱私權功能（包括同意管理、資料保留設定和使用者角色）之外，我們還將以資料處理者的角色利用此機會加入其他功能，以協助您做好準備，以利您擔任資料控制者，處理特定GDPR請求：<br /> 
    <ul> 
  ...
  <tr> 
   ...
   <td> Adobe Campaign的全新Creative Designer在Campaign中提供完全整合的建立體驗，讓您輕鬆快速地建立吸引人、個人化的電子郵件，而不需要編寫程式碼的一行。透過其強大的拖放介面，不論使用者是從空白顯示板開始，還是運用現有的內容片段或範本，Creative Designer都能協助縮放電子郵件建立作業。<br />主要功能包括：<br /> 
    <ul> 
     <li> 透過拖放介面，利用原生Creative Cloud整合強化，以視覺化方式設計和建立完全個人化、回應式電子郵件 </li> 
  ...
  <tr> 
   <td> 多語言推播傳遞<br /> </td> 
   <td> 電子郵件和簡訊頻道中已存在的相同簡單多語言介面已新增至推播頻道，不論客戶偏好的語言為何，都能協助您與客戶互動。<br /> 此功能為管理跨多個區域的推播行銷活動，並想要以偏好語言鎖定使用者的客戶提供可擴充的自動解決方案。 它可讓您按一下滑鼠，即可透過範本化試算表將所有語言變體上傳至單一推送傳送。 Adobe Campaign接著會根據使用者的語言偏好執行自動分段，透過簡化工作流程和報表，協助減少重複專案。<br /> 如需詳細資訊，請參閱<a href="../../channels/using/creating-a-multilingual-push-notification.md">詳細檔案</a>.<br /> </td> 
  </tr> 
  <tr> 
   <td> 在異動訊息傳遞中使用自訂資源<br /> </td> 
   <td> 除了現成可用的欄位之外，異動訊息現在可讓您使用自訂資源來擴充訊息的內容。<br />例如：<br /> 
    <ul> 
...
* 新增可在傳送傳遞後即時更新&#x200B;**[!UICONTROL Delivered]**&#x200B;或&#x200B;**[!UICONTROL Bounces + Errors]** KPI的選項。 系統會直接從提供者接收的SR （狀態報告）重新計算事件。
...
* 修正無法自動選取A/B測試電子郵件獲勝者的問題，導致未傳送電子郵件。 如果傳送處於&#x200B;**[!UICONTROL retryInProgress]**&#x200B;狀態，就可能發生此行為。
* 修正在重新開啟A/B測試電子郵件的引數時，可能導致出現錯誤訊息的問題。

_對象和查詢_

* 修正無法存取資料以及為從Adobe Campaign Classic復寫至Standard的收件者設定查詢的問題。
* 修正使用&#x200B;**計數**&#x200B;或&#x200B;**預覽**&#x200B;按鈕後，在查詢編輯器中使用篩選器型別欄位時發生的問題。

_工作流程_

* 已最佳化&#x200B;**帳單**&#x200B;工作流程，以改善傳遞準備延遲。
* 修正使用循環傳送活動時，無法在出站轉變中顯示母體資料的問題。
* 修正了在&#x200B;**更新資料**&#x200B;活動後，拒絕記錄無法顯示在轉換中的問題。
* 修正可能導致&#x200B;**deliverabilityUpdate**&#x200B;技術工作流程失敗的問題。

_整合_

* 修正無法將國際字元正確傳送至Adobe Analytics的問題。
* Assets現在嘗試從訊息的Experience Cloud資產庫插入影像時，應該會載入得更快。
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <strong>訂閱服務</strong>工作流程活動現在可讓您訂閱或取消訂閱多項服務的設定檔清單。 在工作流程中，匯入包含設定檔的檔案，並針對每個設定檔，匯入操作型別和服務。 <strong>訂閱服務</strong>活動將能夠使用此資訊，並一次動態處理您所有的設定檔訂閱和取消訂閱。<br /> 如需詳細資訊，請參閱<a href="../../automating/using/subscription-services.md">詳細檔案</a>.<br /> </td> 
  </tr> 
  <tr> 
...
* 修正方案下拉式清單無法顯示&#x200B;**[!UICONTROL Offers]**&#x200B;連結的問題。
...
* 修正&#x200B;**[!UICONTROL Report sharing]**&#x200B;技術工作流程在達到到期限制之前發生的錯誤。

_工作流程_

* 修正拖放活動後無法存取活動的問題。
* 修正可能導致&#x200B;**[!UICONTROL Segmentation]**&#x200B;活動之輸出轉換順序在某些情況下發生變更的問題。
* 修正讀取包含列舉型別欄位且先前已從工作流程儲存的對象時發生的錯誤
* 修正在工作流程中建立之傳送的排程屬性定義時，即使取消核取&#x200B;**[!UICONTROL Request confirmation before sending messages]**&#x200B;選項，仍會保持核取狀態的問題。
* 透過&#x200B;**[!UICONTROL Additional data]**&#x200B;索引標籤中的新選項，現在可以在&#x200B;**[!UICONTROL Query]**&#x200B;活動中停用自動移除重複資料列（DISTINCT子句）。 基於效能考量，建議在定義許多（超過100個）其他元素時停用此選項。

_整合_

* 已對&#x200B;**[!UICONTROL Data sources]**&#x200B;設定畫面進行一些改良。
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td> 疲勞管理報告<br /> </td> 
   <td> 疲勞管理報告是專用的可設定報告，可顯示疲勞規則在傳送前指定日期範圍內對電子郵件、推播、簡訊和直接郵件通道之傳送的影響。 由於新增的insight能以單一檢視快速檢視所有衝突行銷活動，行銷人員得以根據更有效設定疲勞規則來規劃行銷活動，並排定通訊的優先順序。<br /> 如需詳細資訊，請參閱<a href="../../sending/using/fatigue-rules.md#viewing-the-fatigue-rule-summary-report">詳細檔案</a>.<br /> </td> 
  </tr> 
  <tr> 
   <td> 報告共用<br /> </td> 
   <td> 「報表共用」可讓您以電子郵件附件的形式與Adobe Campaign使用者共用報表，包括自動循環共用報表。 收到週期性報表的使用者可以透過每封電子郵件中的專用連結取消訂閱這些通訊。<br /> 如需詳細資訊，請參閱<a href="../../reporting/using/reporting-interface.md#share-tab">詳細檔案</a>.<br /> </td> 
  </tr> 
  <tr> 
   <td> 推播新功能<br /> </td> 
   <td> 推送訊息預覽 — 從推送通知內容編輯器預覽在iOS和Android裝置上顯示的推送通知，在測試或執行傳遞前可先檢視收件者所見到的內容。<br />如需詳細資訊，請參閱<a href="../../channels/using/preparing-and-sending-a-push-notification.md#preparing-the-notification">詳細檔案</a>。<br />可用的內容 — 如果使用者在一段時間後都未開啟應用程式，其資料可能會過期。這會導致在使用者最終開啟應用程式時，必須更新或取代資料，這可能會造成使用應用程式發生延遲。透過新增的「可用內容」支援，Adobe Campaign使用者在傳送推播通知時，可以喚醒其應用程式以在背景重新整理其資料，以提升一致性和控制使用者的應用程式內體驗。<br />可變內容 — 透過新增的「可變內容」支援，Adobe Campaign使用者現在可以運用其行動應用程式延伸模組，進一步修改來自Adobe Campaign之推播通知的內容或呈現方式。例如，使用者可利用可變內容來： <br /> 
    <ul> 
     <li> 解密以加密格式傳送的資料 </li> 
...
* **[!UICONTROL Scheduler]**&#x200B;活動現在允許您根據收件者的時區傳送傳遞。
* SMS：資料庫中的選項&#x200B;**[!UICONTROL Store incoming MO]**&#x200B;已新增到外部帳戶。 選取後，所有傳入的簡訊將儲存到&#x200B;**inSMS**&#x200B;資料表中。
...
* 為推播通知預覽視訊新增暫停按鈕。
* 推播通知預覽現在可供不同裝置使用，例如iPhone、Android、平板電腦。

_報告_

* 修正顯示超過100%的費率的錯誤。
* 修正使用者無法以CSV格式下載報表的問題。
* 已在首頁中新增新的&#x200B;**[!UICONTROL Report]**&#x200B;專案。
