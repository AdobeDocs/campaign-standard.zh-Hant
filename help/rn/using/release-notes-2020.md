...
![](assets/do-not-localize/cp-icon.png) **新的控制面板 6 月版本**，包含活躍輪廓監控、子網域傳遞送能力稽核及 GPG 金鑰管理。 [了解更多資訊](https://experienceleague.adobe.com/docs/control-panel/using/release-notes.html?lang=zh-Hant)。

![](assets/do-not-localize/cp-icon.png) **新控制面板 10 月發行版本**，其中包含使用 CNAME 的網域設定及新的資料庫監控功能。 [了解更多資訊](https://experienceleague.adobe.com/docs/control-panel/using/release-notes.html?lang=zh-Hant)。
...
<tbody> 
<tr> 
  <td> <p>您現在可以使用<strong>控制組組</strong>，透過排除其客群的一部分來評估行銷活動的影響。 然後，您將能夠將接收到消息的目標群體的行為與未作為目標的聯絡人的行為進行比較。 根據傳送日誌，您也可以在未來的行銷活動中鎖定控制組。
</p>
...
* **隱私權管理**：**CCPA選擇**&#x200B;退出欄位現在透過 Campaign 介面和 API 提供，現在也可以透過隱私權核心服務提供支援。 此欄位可讓 Adobe Campaign 使用者追蹤消費者是否已選擇退出個人資訊的出售。 [了解更多](https://helpx.adobe.com/content/help/tw/campaign/kb/acs-privacy.html#ccpa)
...
* **簡訊錯誤管理**&#x200B;已得到改善，以防止將過多的輪廓新增到隔離清單中。 依預設，簡訊錯誤現在會設定為軟錯誤，而非硬錯誤。 請參見[此頁面](https://helpx.adobe.com/tw/campaign/kb/sms-connector-protocol-and-settings.html)。
...
* 修正&#x200B;**內容區塊**&#x200B;和&#x200B;**片段**&#x200B;搜尋列的問題。 使用其中一個搜尋列時，僅當按一下&#x200B;**顯示更多結果……**&#x200B;時，才會顯示結果。(CAMP-44205)
...
* 已新增重試機制，以防止嘗試使用&#x200B;**讀取客群**&#x200B;活動匯入 Experience Platform 客群時發生錯誤。 (CAMP-43947, CAMP-43366)
* 現在會自動設定組織單位，以符合建立輪廓或實體之使用者的組織單位。 不得再移除組織單位並將其留空白。
...
* 修正使用包含列舉類型欄位且連結至&#x200B;**聯合**&#x200B;或&#x200B;**擴充**&#x200B;活動的&#x200B;**讀取客群**&#x200B;活動時發生的問題。 (CAMP-42795)
...
* 修正選取要用於輪廓替代內容的輪廓時，可能會顯示逾時錯誤的問題。 (CAMP-44746)
* 修正部署包含錯誤連結定義的自訂資源後，執行個體無法運作的問題。 (CAMP-44406)
* 修正建立空白連結實體（型別、品牌等）的問題 將傳遞複製並貼上行銷活動範本後。 (CAMP-44765)
...
* 修正將輪廓替代與 targetData 個人化搭配使用時的錯誤。 (CAMP-44996)
* 修正匯出套件中的傳送範本時，傳送預覽失敗的問題。 (CAMP-44084)
* 修正使用自訂目標對應時，無法將校樣傳送至測試輪廓的問題。 (CAMP-43701)
* 修正使用&#x200B;**讀取客群**&#x200B;活動並使用&#x200B;**輪廓**&#x200B;以外的目標選擇維度選擇目標客群時，在工作流程中發生的錯誤。  (CAMP-41885)
...
* 修正使用自訂欄位將&#x200B;**訂閱擴充至具有輪廓連結的應用程式** (appSubscriptionRcp) 結構描述的問題。 未自動建立索引，這可能會影響推播傳送時間。 (CAMP-41120)
...
* **由 AI 支援的傳送時間最佳化和輪廓計分** - 您現在可以最佳化客戶歷程的設計和傳遞，以預測每個人的參與偏好。 Adobe Campaign 採用 Journey AI，可根據歷史參與量度來分析和預測開放率、最佳傳送時間和可能的流失率。 [了解更多](../../sending/using/predictive.md)
* **巴西的新隱私權規範** — 除了Campaign中已提供的隱私權功能以外，Adobe協助您做好準備，以迎接巴西的Lei Geral de Proteçao de Datos (LGPD)。 建立隱私權要求時，LGPD 規則類型已新增至 Adobe 隱私權核心服務。 [了解更多](https://helpx.adobe.com/tw/campaign/kb/campaign-privacy-overview.html)

**功能改進**

* 在 **Prefix** 欄位中可用於[使用目標輪廓之測試訊息](../../sending/using/testing-messages-using-target.md)的字元數，已經從 32 增加為 500 個字元。
...
* 隨「傳輸檔案」活動下載的檔案現在會在 X 天後刪除，其中 X 由「工作流程」屬性中 **Execution** 功能表的 **History in days** 欄位決定。 [閱讀更多](../../automating/using/managing-execution-options.md)

**體驗平台整合**

* 從&#x200B;**讀取對象**&#x200B;活動啟動Adobe Experience Platform對象已經有所改善，而可提供較出色的效能及穩定性。 此外，工作流程記錄檔已經更清楚且更詳細地記錄啟動工作，讓您在讀取 Adobe Experience Platform 客群時，能夠更輕鬆地進行監控和疑難排解。

**修補程式**

* 修正導致在自訂資源的發佈工作期間建立 Ghost 資源的錯誤。
* 修正當輪廓資源已擴充至自訂資源時，可能無法顯示輪廓行銷歷史記錄的問題。 (CAMP-41009)
* 已修正開啟編輯器時，以法文顯示其內容的現成可用登錄頁面範本問題。 (CAMP-41639)
* 修正動態內容的推播通知無法顯示表情符號的問題。 (CAMP-40715)
* 修正&#x200B;**重複資料刪除**&#x200B;活動，可能導致將錯誤的段代碼分配給其中一個出站補集轉變的問題。 (CAMP-41400)
...
* 修正工作流程中，使用數個&#x200B;**儲存客群**&#x200B;活動來建立客群時可能會發生的問題，這些活動會補充外站轉變。 (CAMP-39992)
* 修正在交易電子郵件中使用個人化時造成資料不一致的問題。 (CAMP-41842)
* 修正刪除推播通知傳送中的自訂欄位時發生的問題。 (CAMP-37586)
* 修正使用者無法變更報告的錯誤。 (CAMP-42505)


![](assets/do-not-localize/cp-icon.png) **新的控制面板可能會與 CNAME 子網域發行**&#x200B;憑證續約。 [了解更多資訊](https://experienceleague.adobe.com/docs/control-panel/using/release-notes.html?lang=zh-Hant)。
...
<table> 
 <thead> 
  <tr> 
   <th> <strong>使用目標輪廓進行電子郵件測試</strong><br /> </th> 
  </tr> 
 </thead> 
 <tbody> 
  <tr> 
...
>[!NOTE]
>
>新功能將於 4 月在「Campaign 控制面板」中發佈，包括 Google TXT 記錄管理、資料庫空間監控和電子郵件警報。 有關這些功能的詳細資訊，請參閱[控制面板發行說明](https://experienceleague.adobe.com/docs/control-panel/using/release-notes.html?lang=zh-Hant)。

**功能改進**

* 交易式訊息使用者體驗已增強，介面一致性也已獲改善。 [閱讀更多](../../channels/using/getting-started-with-transactional-msg.md)
* Campaign Standard 現在可讓您使用工作流程的其他資料，將校樣傳送至測試輪廓。
* 外部 API 活動的護欄已更新。 [閱讀更多](../../automating/using/external-api.md)
...
* 修正匯入已從資料庫刪除的 Adobe Experience Platform 輪廓時可能發生的問題。
* 修正工作流程記錄中，針對匯入記錄總數顯示錯誤結果的問題。

**修補程式**

* 修正在 **Alias** 欄位中新增空格後，接著建立新列項目時，**擴充**&#x200B;工作流程活動可能發生的問題。 (CAMP-39229)
* 修正傳送校樣訊息時，每個測試輪廓都可定位的問題。
* 修正取消發佈和刪除事件設定後發生的問題。 [閱讀更多](../../channels/using/publishing-transactional-event.md#deleting-an-event)
...
* 修正在現有工作流程中開啟&#x200B;**讀取客群**&#x200B;活動的問題。 (CAMP-41002)
...
<table> 
 <thead> 
  <tr> 
   <th> <strong>客群目標 (測試版) </strong><br /> </th> 
  </tr> 
 </thead> 
 <tbody> 
  <tr> 
   <td> <p>客群目標可讓您將 Adobe Experience Platform 的細分群體共用給 Adobe Campaign。</p>
    <p>請注意，此功能僅適用於Azure上代管的客戶。</p>
   </td> 
  </tr> 
 </tbody> 
...
* Enhanced MTA 的全球可用性：訊息（包括交易訊息）現在由 Adobe Campaign Enhanced MTA 傳送，此 MTA 提供升級的傳送基礎架構，可改善傳送能力、吞吐量和退信處理。 [閱讀更多](https://helpx.adobe.com/tw/campaign/kb/campaign-enhanced-mta.html)
...
* 現在，透過將 `_forcePagination=true` 參數新增至呼叫 URL，您可在使用大型表格時，使用 Campaign Standard API 來執行分頁。 [閱讀更多](../../api/using/pagination.md)

* 「傳送記錄檔 ID」（每個記錄檔的唯一識別碼）現在可用於所有定位維度的「傳送記錄檔」和「追蹤記錄檔」資源。 舉例來說，這可在匯出時識別傳送或追蹤記錄檔。 [閱讀更多](../../automating/using/exporting-logs.md)
