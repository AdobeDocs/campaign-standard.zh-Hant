...
若要在 Adobe Campaign 中執行此動作，請在定義傳送目標時，建立<b>控制組</b>。 輪廓會隨機新增至控制組、篩選或不篩選，或根據條件。

然後，您將能夠將接收到消息的目標群體的行為與未作為目標的聯絡人的行為進行比較。 根據傳送日誌，您也可以在未來的行銷活動中鎖定控制組。

<!--The control group is built when the delivery is prepared.-->

## 概觀 {#overview}

可以從主要目標隨機擷取控制組和/或從特定群體中選取。 因此，定義控制組的主要方法有兩種：
* 從主要目標&#x200B;**擷取**&#x200B;多個輪廓。
* 根據查詢中定義的條件&#x200B;**排除**&#x200B;一些輪廓。

定義控制組時，可以使用這兩種方法。

在傳送準備步驟中屬於控制組的所有輪廓將會從主要目標中移除。 訊息傳送後，他們將無法接收。

## 從目標人口中擷取 {#extraction-target-population}

若要定義控制組，您可以選擇從目標群體中隨機或基於排序、百分比或固定數量的輪廓中擷取。

### 目標擷取 {#target-extraction}

首先，定義從目標中擷取輪廓的方式： **隨機**&#x200B;或根據&#x200B;**排序**。

在 **[!UICONTROL Target extraction]** 部分下，選擇以下選項之一：

* **[!UICONTROL Random sampling]**：在準備傳送時，Adobe Campaign 將隨機擷取與百分比或您設定為[大小限制](#size-limit)的最大數量對應的多個輪廓。

  例如，如果您接著在 **[!UICONTROL Limits]** 部分中將臨界值設為 10，則控制組將從目標群體隨機選取10%。<!--Change screenshot to match example)-->
...
* **[!UICONTROL Keep only the first records after sorting]**：此選項可讓您根據一或多個排序順序定義限制。
  ...
  定義包含很少或頻繁購買的輪廓的控制組，並將他們的行為與已聯絡的收件人行為進行比較，這可能很有趣。
...
無論您選取 **[!UICONTROL Random sampling]** 或 **[!UICONTROL Keep only the first records after sorting]**，您必須設定如何限制從主要目標擷取的輪廓數量。 執行下列任一項項目：

* 選取 **[!UICONTROL Size (as a % of the initial population)]**，並填入對應的框架。

  例如，如果您設定 10，則根據以上所選取的選項，Adobe Campaign 會：
   * 隨機擷取目標群體的 10%。
   * 如果您選取 **[!UICONTROL Age]** 欄位作為排序標準，請從目標群體中擷取最舊 10% 的輪廓。

  >[!NOTE]
  >
  >如果您取消核取 **[!UICONTROL Descending sort]** 選項，將會擷取 10% 最年輕的輪廓。

* 選取 **[!UICONTROL Maximum size]**，並填入對應的框架。

  例如，如果您設定 100，Adobe Campaign 將會：
   * 從目標群體中隨機擷取 100 個輪廓。
   * 如果您選取 **[!UICONTROL Age]** 欄位作為排序標準，則從目標群體中擷取 100 個最舊的輪廓。

  >[!NOTE]
  >
  >如果您取消核取 **[!UICONTROL Descending sort]** 選項，將會擷取 100 個最年輕的輪廓。

## 排除特定人口 {#excluding-specific-population}

定義控制組的另一種方法是使用査詢從目標中排除特定的群體。
...
以下範例說明如何使用這兩種方法定義控制組：從主要目標擷取輪廓，以及使用查詢排除特定群體。
...
1. 將 100 設定為最大大小。 將從目標中擷取 100 個最舊的輪廓。

1. 在 **[!UICONTROL Target exclusion]** 區段中，根據您使用[查詢編輯器](../../automating/using/editing-queries.md)選擇的條件，定義將從目標中排除的輪廓。 例如，「年齡小於 20 歲」。
...
擷取的輪廓 (100 個最年長的輪廓) 和根據査詢定義的輪廓 (20 歲以下的輪廓) 將從主要目標中撤回。 他們並不會收到訊息。
...
>[!IMPORTANT]
>
>若要存取傳遞記錄，您必須擁有[系統管理員角色](../../administration/using/users-management.md#functional-administrators)，並且是&#x200B;**[!UICONTROL All]** [組織單位](../../administration/using/organizational-units.md)的一部分。<!--If you would like to restrict access for a particular user or group of users, do not link it to **[!UICONTROL All]** units.-->

### 檢查傳送記錄 {#checking-logs}

查看在發送訊息後從目標中刪除的輪廓，請檢查 **[!UICONTROL Delivery logs]**。 如需傳送記錄的詳細資訊以及存取傳送記錄的方法，請參閱[本節](../../sending/using/monitoring-a-delivery.md#delivery-logs)。

* 在 **[!UICONTROL Sending logs]** 索引標籤中，您可以查看已擷取和已排除的輪廓。 他們擁有 **[!UICONTROL Ignored]** 狀態，並且 **[!UICONTROL Control group]** 為失敗的原因。

  ![](assets/control-group-sending-logs.png)

* 您也可以檢查 **[!UICONTROL Exclusion causes]** 索引標籤，以查看傳送中未包含的輪廓數量。

  ![](assets/control-group-exclusion-causes.png)

### 使用控制組日誌 {#using-logs}

傳送後，您可以使用傳送記錄來篩選未接收訊息的輪廓。 請遵循以下步驟：
...
若要根據未收到訊息的輪廓進行目標選擇，您也可以使用傳送記錄。 請遵循以下步驟：
...
現在，您可以鎖定未收到第一條訊息的輪廓，因為它們是控制組的一部分，並向他們發送另一封電子郵件。

在相同的工作流程中，您也可以建立另一個查詢來鎖定已收到電子郵件的輪廓，並向其發送不同的訊息。
