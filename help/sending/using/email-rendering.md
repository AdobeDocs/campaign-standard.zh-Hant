...
1. 使用查詢編輯器定義您要使用的測試輪廓，包括用於&#x200B;**電子郵件呈現**&#x200B;的測試輪廓。 請參閱[關於測試輪廓](../../audiences/using/managing-test-profiles.md)。
...
1. 選取 **[!UICONTROL Email rendering]** 選項，然後按一下 **[!UICONTROL OK]**。
   ...
   >[!NOTE]
   >
   >**[!UICONTROL Proof + Email rendering]** 選項可讓您在傳送校樣並同時使用電子郵件呈現功能。 您可以讓校樣收件者核准您的郵件，同時也可以根據目標收件匣來測試接收郵件的方式。 在此情況下，您也需要選取「校樣測試輪廓」。 請參閱[關於測試輪廓](../../audiences/using/managing-test-profiles.md)。
