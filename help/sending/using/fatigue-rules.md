---
title: 疲勞規則
description: 建立疲乏規則，以管理使用輪廓的過度通訊。
...
疲勞規則可讓行銷人員設定全球跨通道企業規則，自動排除行銷活動中過度請求的輪廓。

若要實施疲勞規則，請定義每個輪廓的最大訊息數量，並選取將套用該規則的期間。 在準備傳送期間，輪廓會根據已傳送給輪廓的訊息數量，從傳送中排除（如果適用）。
...
* [最佳化通訊頻率以防止聯絡疲勞](https://helpx.adobe.com/tw/campaign/kb/simplify-campaign-management.html#Engageyourcustomersateverystep)
...
1. 在 **[!UICONTROL General]** 索引標籤中，定義計算每個輪廓最大訊息數量的方法。 您可以選取常數臨界值或變數。 您也可以精簡輪廓與傳送的臨界值。 如需詳細資訊，請參閱[定義臨界值](#defining-the-threshold)。
...
在準備傳送期間，輪廓會視情況從傳送中排除，且是根據已傳送給輪廓的傳送數量而定。 您可以在傳送記錄檔中檢視疲勞規則執行結果。 請參閱[檢視疲勞結果](#viewing-the-fatigue-results)。
...
* 行動裝置（簡訊）
* 直接郵件
* 行動應用程式：此通道可讓您您傳送推播通知給輪廓或應用程式訂閱者。 如果您選取傳送通知給輪廓，則通知會與多通道疲乏規則相容。

  >[!IMPORTANT]
  >
  >疲勞規則與傳送給應用程式訂閱者的推播通知不相容。 如果您要傳送訊息給應用程式訂閱者，則不會套用疲勞規則。

* 所有通道：此選項可讓您將規則套用至所有通道。 例如，您可以決定每個月在任何通道上最多傳送 3 則訊息。 如果您在上週傳送 2 封電子郵件給輪廓，而您今天嘗試傳送推播通知，則會排除相同的輪廓。

**傳送類型**

疲勞規則與所有傳送類型相容：一次性傳送、重複傳送、工作流程傳送與交易式訊息。

**交易式訊息**&#x200B;可以用來傳送以事件 (rtEvent) 為目標的服務訊息，以及行銷訊息（選擇目標輪廓），例如再行銷訊息。 疲勞規則僅與行銷訊息相容（選擇目標輪廓）。 事件交易式訊息不包含輪廓資訊，因此它們與疲勞規則不相容（即使在輪廓擴充的情況下亦然）。 透過交易式訊息中的行銷訊息支援，您可以&#x200B;**將疲乏規則套用至所有通道，包括行銷交易式訊息**。

## 定義臨界值 {#defining-the-threshold}

每個疲勞規則會定義一個臨界值，即在指定期間內可以發送到一個輪廓的最大消息數量。 一旦達到此臨界值時，在考慮到該期間結束之前，將不再進行傳送。 此程序可以讓您在訊息超過設定的臨界值時，自動將輪廓排除在傳送之外，以避免過度請求。

臨界值可以是常數或變數。 這表示在指定期間，臨界值可能會因某個輪廓而異，甚至會因為相同的輪廓而有所不同。

**使用修正臨界值**

該臨界值表示在相關期間內可發送到輪廓最大的訊息數量。
...
* 選取輪廓欄位：每個輪廓的臨界值會依所選欄位而有所不同。 例如，如果您已使用　&#39;Communication frequency&#39;　欄位擴展輪廓資源，請按一下 **[!UICONTROL Threshold computation formula]** 欄位右側的按鈕，然後選取您的欄位。 對於每個輪廓，臨界值將採用　&#39;Communication frequency&#39;　欄位的值。

  ![](assets/fatigue21.png)

* 定義公式：按一下 **[!UICONTROL Threshold computation formula]** 欄位右側的第二個按鈕，以定義進階臨界值計算公式。 例如，您可以根據輪廓所屬的區段來索引授權訊息的數量。 這表示屬於　&#39;Web&#39;　區段的輪廓可能會收到比其他輪廓更多的訊息。 **[!UICONTROL Iif (@origin='Web', 5, 3)]** 類型公式可以授權傳送 5 則訊息至網頁區段的輪廓，而其他區段則授權傳送 3 則訊息。

  ![](assets/fatigue14.png)

**精簡輪廓與傳送的臨界值**

預設情況下，所有訊息都將考慮到臨界值計算。 核取 **[!UICONTROL Refine Threshold on profiles and deliveries]** 方塊以篩選輪廓與傳送，以便在準備傳送時計算。

在以下範例中，只會計算男性輪廓，且只會計算以　**Newsletters**　開頭之索引標籤的傳送。
...
在準備傳送期間，輪廓會視情況從傳送中排除，且是根據已傳送給輪廓的傳送數量而定。 若要檢視疲勞規則執行結果，請按一下 **[!UICONTROL Deployment]** 區塊右下角的按鈕。
...
* 使用套用至&#x200B;**所有通道**&#x200B;的&#x200B;**常數臨界值**，建立疲勞規則：
  ...
  上週，您的進階輪廓收到促銷電子郵件與交易式再行銷電子郵件。 您也排程了下週傳送的 簡訊。 今天，您決定傳送以所有輪廓為目標的推播通知。 進階輪廓將會排除在今天的推播之外，因為已達到其在 2 週期間的訊息數量上限。

  ![](assets/fatigue23.png)

* 根據&#x200B;**變數臨界值**，使用&#x200B;**輪廓欄位**&#x200B;建立疲勞規則：

  您已使用「通訊限制」欄位擴充輪廓資源，以定義每個輪廓的不同臨界值。 在疲勞規則中，根據此欄位定義變數臨界值，並選取滑動期間為 2 天。 舉兩個輪廓之範例：John 的通訊限制為 1，David 的臨界值為 2。 昨天，他們兩個都收到了電子報電子郵件。 您決定今天再傳送一封電子郵件給他們。 只有 David 會收到，因為 John 已被排除在目標之外。

  ![](assets/fatigue24.png)

* 使用&#x200B;**臨界值計算公式**&#x200B;建立疲勞規則：

  您想要根據輪廓的年齡變更臨界值。 如果輪廓低於 40，您想要定義限制為 4，而舊輪廓的限制為 2。 您可以直接在疲勞規則中建立根據輪廓的年齡計算臨界值的公式，而不是使用擴充欄位為每個輪廓定義之臨界值。 在我們的例子中，公式為 **[!UICONTROL Iif (@age<40, 4, 2)]**。
...
* 建立疲勞規則，可在輪廓及傳送上&#x200B;**精簡臨界值**：

  您已使用　&#39;Score&#39;　欄位擴充輪廓資源，並且已使用　&#39;Type&#39;　欄位擴充傳送資源。 您想要定義常數臨界值為 3，但想從計數中排除　&#39;Alert&#39; 或 &#39;Black Friday&#39;　類型的所有傳送，以及分數大於 10 的所有輪廓。 當規則執行時，會在過去和已排程的傳送中，將所有非　&#39;Alert&#39; 或 &#39;Black Friday&#39;　類型的傳送計入分數小於 10 的輪廓。

  ![](assets/fatigue26.png)

以下是使用臨界值計算公式的疲勞規則的逐步範例。

在此使用案例中，我們想要建立類型規則，以防止每週傳送超過 2 則訊息給進階輪廓，以及每週傳送 2 則訊息給標準輪廓。

為了識別客戶與潛在客戶，我們擴展了輪廓資源，**[!UICONTROL Status]** 欄位包含 0 個進階輪廓和 1 個標準輪廓。

若要建立規則，請套用以下步驟：

1. 建立新的&#x200B;**疲勞類型**&#x200B;類型規則。
1. 在 **[!UICONTROL Threshold]** 區段中，我們要建立一個公式以根據每個輪廓計算臨界值。 在 **[!UICONTROL Depends on the recipient]** 欄位中選取 **[!UICONTROL Threshold type]** 值，然後按一下 **[!UICONTROL Threshold computation formula]** 欄位右側的第二個按鈕圖示。
...
1. 然後，在 **[!UICONTROL Available fields]**　區段中選取配輪廓的 **Status**。
