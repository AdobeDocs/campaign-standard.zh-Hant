...
* **預測性傳送時間最佳化**&#x200B;針對電子郵件開啟或點按，以及推送訊息開啟，預測每個收件者設定檔的最佳傳送時間。 對於每個收件者輪廓，分數會指出每個工作日的最佳傳送時間，以及哪個工作日最適合傳送以取得最佳結果。
...
預測性傳送時間最佳化可預測每個電子郵件開啟或點按，以及推播訊息開啟的收件者設定檔的最佳傳送時間。 對於每個收件者輪廓，分數會指出每個工作日的最佳傳送時間，以及哪個工作日最適合傳送以取得最佳結果。
...
在實作至 Campaign 以後，機器學習功能便能運用新的索引標籤，以最佳的開啟/點按分數豐富輪廓資料。 系統會使用技術工作流程運算量度，並將其帶入Campaign。
...
依預設，輪廓分數會提供一週中每一天的最佳時間，以及一週中最佳的整體時間。
...
* **保留的百分位數**：輪廓排名。 值範圍從 0.0 到 1.0。 例如，如果保留百分比排名是0.953，則此收件者很可能堅持使用品牌，而取消訂閱的可能性則低於所有收件者的95.3%。
