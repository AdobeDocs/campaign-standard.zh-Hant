...
1. 設定您的訊息，然後啟動 **準備** 階段。
1. **選取一或多個設定檔** 在訊息所定位的設定檔中。
1. 與每個設定檔建立關聯a **替代地址** 會將校樣傳送至哪個位置。
1. （選用）為每個設定檔定義 **前置詞** 來加到校樣主旨行。
1. **預覽** 在電子郵件設計工具中，如何針對設定檔顯示訊息。
1. 傳送校樣。
...
1. 在訊息控制面板中，確認訊息準備成功，然後按一下 **[!UICONTROL Audience]** 封鎖。

   ![](assets/substitution_preparation.png)

1. 在 **[!UICONTROL Profile substitutions]** ，按一下 **[!UICONTROL Create element]** 按鈕，以選取要用於測試的設定檔。
...
1. 選取要用於測試的設定檔，然後在 **[!UICONTROL Address]** 欄位，然後按一下 **[!UICONTROL Confirm]**. 所有以設定檔為目標的校樣都會傳送至此電子郵件地址，而非此設定檔的資料庫中定義的校樣。

   如果您想要在校樣的主旨行中新增特定首碼，請填入 **[!UICONTROL Subject line prefix]** 欄位。
...
1. 定義所有設定檔和替代地址後，您就可以傳送校樣以測試訊息。 若要這麼做，請按一下 **[!UICONTROL Test]** 按鈕，然後選取要執行的測試類型。

   請注意，如果未將測試設定檔新增至訊息目標，則 **[!UICONTROL Email rendering]** 和 **[!UICONTROL Proof + Email rendering]** 選項不可用。  如需校樣傳送的詳細資訊，請參閱 [本節](../../sending/using/sending-proofs.md).
...
此外，如果您的對象已設定位址和首碼欄，您就可以在 **[!UICONTROL Profile substitutions]** 標籤。 透過替代地址匯入對象的範例，在 [本節](#use-case).
...
1. 選取要使用的對象，然後輸入要用於傳送給對象的校樣的替代地址和前置詞。
   ...
   如果要使用的替代地址和/或前置詞已在您的對象中定義，請選取 **[!UICONTROL From Audience]** 選項，然後指定用於檢索這些資訊的列。

   ![](assets/substitution_fromaudience.png)

1. 按一下 **[!UICONTROL Import]** 按鈕。來自與訊息目標對應之對象的設定檔會新增至 **[!UICONTROL Profile substitution]** 頁簽，以及相關的替代地址和前置詞。
...
若要使用目標設定檔預覽訊息，請確定您已將這些設定檔新增至 **[!UICONTROL Profile substitution]** 清單(請參閱 [定義配置檔案和替代地址](#selecting-profiles))。

如果您想在訊息中使用個人化欄位，則必須新增這些欄位 **befor** 啟動訊息準備。 否則，預覽不會將其納入考量。 因此，如果個人化欄位有任何變更，請務必再次啟動訊息準備。
...
1. 選取 **[!UICONTROL Preview]** ，然後按一下 **[!UICONTROL Change profile]**.

   ![](assets/substitution_preview_changeprofile.png)

1. 按一下 **[!UICONTROL Profile Substitution]** 頁簽，顯示已添加以進行測試的替代配置檔案。
...
1. 建立工作流程以從檔案建立對象。 若要這麼做，請新增並設定下列活動：
   ...
   * **[!UICONTROL Reconciliation]** 活動：將檔案中的資訊連結到資料庫中的資訊。 在此範例中，使用設定檔的電子郵件地址作為調解欄位(如需此活動的詳細資訊，請參閱 [本節](../../automating/using/reconciliation.md))。
   * **[!UICONTROL Save audience]** 活動：根據匯入的檔案建立對象(如需此活動的詳細資訊，請參閱 [本節](../../automating/using/save-audience.md))。

   ![](assets/substitution_uc2.png)

1. 執行工作流程，然後前往 **[!UICONTROL Audiences]** 標籤，以檢查對象是否已建立且包含所需資訊。
...
1. 新增 **[!UICONTROL Query]** 和 **[!UICONTROL Email delivery]** 活動，然後根據您的需求進行設定(請參閱 [查詢](../../automating/using/query.md) 和 [電子郵件傳送](../../automating/using/email-delivery.md) 區段)。
...
1. 開啟 **[!UICONTROL Email delivery]** 活動。 在訊息控制面板中，按一下 **[!UICONTROL Audience]** 封鎖。

   ![](assets/substitution_uc5.png)

1. 選取 **[!UICONTROL Profile substitutions]** ，然後按一下 **[!UICONTROL Import from an audience]**.
...
1. 按一下 **[!UICONTROL Import]** 按鈕。對象中的設定檔會新增至清單，及其相關聯的替代地址和主旨行前置詞。
   ...
   >[!NOTE]
   >
   >在我們的案例中，來自對象的所有設定檔皆會以 **[!UICONTROL Query]** 活動。 如果其中一個設定檔不是訊息目標的一部分，則不會將其新增至清單。
...
1. 按一下 **[!UICONTROL Profile Substitution]** 頁簽，以顯示以前添加的替代配置檔案。
...
1. 校樣會根據 **[!UICONTROL Profile substitutions]** 標籤。
