...
控制建立、執行和追蹤跨頻道行銷活動的整個流程，並根據客戶的輪廓、行為、偏好和需求傳送個人化訊息。 您可以輕鬆協調、建模並自動化整個行銷流程，並與其他 Adobe 解決方案整合。
...
**使用預覽、校樣、電子郵件主旨行測試、電子郵件轉譯等多種測試功能，在傳送訊息之前，分析並檢查訊息內容**。透過最佳化訊息傳送時間（排程、時區管理），確保在正確的時間傳送訊息。 [開始訊息測試和傳送](../../sending/using/get-started-sending-messages.md)
...
**追蹤您的傳送收件者的行為**。 運用工作階段和永久 Cookie 來擷取您資料庫中每個連絡人的追蹤資訊。 監視傳遞（已點按的URL、映象頁面、已開啟的訊息……）的追蹤資訊 透過專用日誌和報告。 [開始使用訊息追蹤](../../sending/using/tracking-messages.md)

**運用動態報告**，概述與訊息或行銷活動相關的特定度量。 動態報告提供可以完全自訂的即時報告，可新增對輪廓資料的存取權，除了功能性電子郵件促銷活動資料 (如開啟和點按) 外，還可依輪廓維度 (例如，性別、城市和年齡) 進行人口統計分析。 透過拖放介面，您可以探索資料、判斷電子郵件促銷活動對最重要客戶細分的執行方式，並評估其對收件者的影響。 [開始使用動態報告](../../reporting/using/about-dynamic-reports.md)
