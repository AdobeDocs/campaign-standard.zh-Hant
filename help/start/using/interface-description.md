---
title: 介面說明
description: 瞭解如何導覽及使用Adobe Campaign工作區：首頁、頂端列、進階功能表與動作列。
audience: start
...
* **[!UICONTROL Create an In-App message]**&#x200B;卡片會帶您前往應用程式內建立助理。 此助理可以讓您選取您要建立的應用程式內訊息型別、定義其屬性、對象和內容。 請參閱[建立應用程式內訊息](../../channels/using/about-in-app-messaging.md)區段。
...
* 此 **[!UICONTROL Customer profiles]** 卡片會直接將您導向至輪廓清單。 在此，您可以諮詢與清單中每個輪廓有關的事件。 請參閱[管理輪廓](../../audiences/using/about-profiles.md)。
...
* 頁面左上角的&#x200B;**Adobe**&#x200B;標誌可讓您存取進階功能與設定。 功能表視您的輪廓與權限。
...
使用&#x200B;**[!UICONTROL Search]**&#x200B;欄位來尋找指引。 搜尋結果包括檔案和說明文章、社群論壇的結果以及影片內容，讓您更輕鬆地存取更多內容，以協助您充分運用應用程式。

三個索引標籤可協助您尋找說明和協助：

1. **[!UICONTROL Help]**&#x200B;索引標籤包含：
   * 透過內容連結快速存取Adobe Campaign Standard檔案。
   * **[!UICONTROL Learning]**&#x200B;連結可讓您存取Adobe Campaign課程庫。
   * **[!UICONTROL Community]**&#x200B;連結可存取專為您有關Campaign的問題提供的論壇。
   * 直接存取：說明中心、客戶服務、Experience Cloud產品狀態、開發人員連線、發行說明、發行計畫和&#x200B;**[!UICONTROL About]**&#x200B;畫面。
1. **[!UICONTROL Support]**&#x200B;索引標籤可讓您開啟支援案例，並透過電話或X （以前稱為Twitter）聯絡我們。
1. **[!UICONTROL Feedback]**&#x200B;標籤可讓您更輕鬆地報告問題或分享您的想法。

## 進階功能表 {#advanced-menu}

按一下每個畫面左上角的&#x200B;**Adobe Campaign**&#x200B;圖示，以顯示進階功能表。 進階功能表可能會依您的合約與使用者權限而有所不同。
...
* **[!UICONTROL Profiles]** - 如需此項目詳細資訊，請參閱[管理輪廓](../../audiences/using/about-profiles.md)區段。
* **[!UICONTROL Test profiles]** - 如需詳細資訊，請參閱[管理測試輪廓](../../audiences/using/managing-test-profiles.md)區段。
...
* **清單**&#x200B;會顯示各種元素，例如方案、行銷活動、設定檔等。可以在&#x200B;**[!UICONTROL Card]**&#x200B;或&#x200B;**[!UICONTROL List]**&#x200B;模式中檢視這些元素。 使用變更模式按鈕，可將一個模式切換到另一個模式。 每個元素都會顯示指標。

  ![](assets/ux_liste.png)

  計數器可讓您瞭解元素的數量。 如果此數字超過 30，您必須按一下此計數器以取得總數。

* **控制面板**&#x200B;顯示連結至活動之所有參數的概觀。 此畫面包含互動式區域，允許您獨立分隔與設定不同的概念。
